...
export async function handleComponentsOg(
  req: Request,
  env: Env,
  ...
  const screenshotUrl = new URL("/api/screenshot", requestUrl);
  ...
  const screenshotResponse = await handleScreenshot(
    new Request(screenshotUrl, {
      headers: req.headers,
    }),
    env,
  );
    ...
    >
      ...
      <div
        style={{
          ...
          border: "2px solid rgb(219, 230, 242)",
          overflow: "hidden",
          paddingTop: isLargeScreenshot
            ? showTwitterFooter
              ? PREVIEW_TOP_PADDING / 2
              : PREVIEW_TOP_PADDING
            : 0,
