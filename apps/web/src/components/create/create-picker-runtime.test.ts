...
describe("create picker runtime", () => {
  test("wires pickers through a local Stimulus controller and shared editor event", () => {
    ...
    expect(pickerSource).toContain(
      'data-action="dropdown-menu:value-change->create-picker#valueChanged"',
    );
    expect(pickerSource).toContain("<DropdownMenuRadioItem");
    expect(pickerSource).toContain("closeOnSelect={false}");
    expect(pickerSource).toContain(
      'defaultValue={name === "menuColor" ? decomposeMenuColor(selectedOption?.value ?? value).color : (selectedOption?.value ?? value)}',
    );
    ...
    expect(controllerSource).toContain(
      'this.getItem(value)?.hasAttribute("data-disabled")',
    );
