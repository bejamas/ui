...
describe("starlight head create prepaint", () => {
  test("injects the create docs-root prepaint script only for /create before route head tags", () => {
    const source = fs.readFileSync(headFile, "utf8");
    const astroConfig = fs.readFileSync(astroConfigFile, "utf8");
    const scriptIndex = source.indexOf("createDocsRootPrepaintScript &&");
    ...
    expect(scriptIndex).toBeLessThan(headMapIndex);
