...
- **Blocks** (e.g. navigation-headers-01, features-01, footer-01) — installed in `apps/web/src/components/blocks/<id>` with their portable folder structure. Any primitives the block depends on are installed automatically in `packages/ui`, and the block's imports are rewritten to the app's `aliases.ui`
