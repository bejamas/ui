...
- **Registry blocks** (when available) — installed in `apps/web/src/components/blocks/<id>` with their portable folder structure. Any primitives the block depends on are installed automatically in `packages/ui`, and the block's imports are rewritten to the app's `aliases.ui`
