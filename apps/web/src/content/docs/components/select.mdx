...
const baseClasses =
  "block w-full select appearance-none relative shadow-xs rounded-md bg-background ps-4 pe-10 py-2 text-sm text-foreground transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring disabled:cursor-not-allowed disabled:opacity-50";
...
const controlId =
  id || name || `select-${Math.random().toString(36).slice(2, 9)}`;
---
...
<style>
  ...
  :where(.select) {
    &::picker(select) {
      ...
      @apply rounded-md my-2 p-1 shadow-sm bg-popover text-popover-foreground;
    }
    &::picker-icon {
      @apply hidden;
    }
...
| variant | `"default" \| "ghost" \| "neutral"` | No | "default" |  |
| optionClass | `string` | No | "" |  |
| class | `string` | No | "" |  |
| showCheckmark | `boolean` | No | true |  |
