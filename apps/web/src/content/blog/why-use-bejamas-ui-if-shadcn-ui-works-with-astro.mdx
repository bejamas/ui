---
title: "Why use bejamas/ui if shadcn/ui works with Astro?"
...
The Astro + `shadcn/ui` version is intentionally not a straw man. Static components such as buttons and cards are server-rendered where possible, while interactive sections hydrate as separate `client:load` islands. That isolates the cost of React and Radix without Next.js overhead.
